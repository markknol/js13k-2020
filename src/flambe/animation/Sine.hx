...
class Sine implements Behavior {
	...
	public function new(start:Float, end:Float, ?speed:Float = 1, ?cycles:Float = 0, ?offset:Float = 0) {
		...
		_count = FMath.HALF_PI + offset * (Math.PI / speed); // Start at the start value plus the seconds to offset.
	...
	public function isComplete():Bool {
		return cycles > 0 && ((_count - FMath.HALF_PI) / Math.PI) * .5 >= cycles;
	}
}
