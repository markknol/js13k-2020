...
import temple.components.FrameDelay;
		...
		if (!isDead) {
			...
			for (i in 0...40) {
				Timeout.create(owner, Math.random() * 0.3, () -> {  
					_game.owner.add(_game.getMoment(_pathComponent.data.color, [10, 10], _pathComponent.position - [Math.random()*50,Math.random()*20], [Random.native.range(-60,60)*3,Random.native.range(-60,60)*3])
